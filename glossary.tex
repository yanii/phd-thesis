...
\newdualentry{cublas}{CuBLAS}{CUDA BLAS}{Nvidia's implementation of \acrshort{blas} for the \acrshort{cuda}}
\newdualentry{cuda}{CUDA}{Compute Unified Device Architecture}{Nvidia's \acrshort{gpu} programming \acrshort{api}}
\newdualentry[see={blas,cpu}]{mkl}{MKL}{Intel's Matrix Kernel Library}{\Glsfmtshort{blas} implementation for Intel \glsfmtshort{cpu}}
...
\newglossaryentry{padding}{name={padding}, description={padding of the input \gls{featuremap}/image for convolution, pads the outer edge of the image with (typically zero) dummy values to allow the convolutional filter to be applied to every input pixel}}
\newglossaryentry{stride}{name={stride}, description={number of rows/columns of the input \gls{featuremap}/image to skip during convolution}}
...
\glsxtrnewsymbol[description={\# channels (input when used with \gls{d}) of a convolutional \gls{filter}/\gls{featuremap}}]{c}{\ensuremath{c}}
\glsxtrnewsymbol[description={\# output channels of a convolutional \gls{filter}/\gls{featuremap}}]{d}{\ensuremath{d}}
