...
\begin{equation}
	y_j = f\left( \sum_i w_{ji} x_{i} \right)
\end{equation}
and assuming the error of the prediction is a sum of squares,
\begin{equation}
E = \sum_n \sum_j (y^n_j - t^n_j).
\end{equation}

Chain rule allows us to calculate the error for each weight $w_{ji}$ in the network,
\mynote{Derivation of delta rule here}
\begin{equation}
\begin{aligned}
    \frac{\partial E}{\partial w_{ij}} &= \frac{\partial E_i}{\partial y_i}\frac{\partial y_i}{\partial w_{ij}} = \frac{\partial E_i}{\partial y_i}\frac{\partial y_i}{\partial f}\frac{\partial f}{\partial w_{ij}},& \textrm{(chain rule)}\\
    & = \frac{\partial (y_i - t_i)}{\partial y_i}\frac{\partial f\left(\sum_j w_{ij} x_{j}\right)}{\partial w_{ij}}, & \\
