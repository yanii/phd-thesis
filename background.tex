...
\caption[The interpretation of a perceptron as a class-separating hyperplane]{The interpretation of a perceptron as a class-separating hyperplane, \ie a line, in $\mathbb{R}^2$.}
...
The delta rule for single-layered neural networks is gradient descent, using the derivative of the network's weights with respect to the output error to adjust the weights to better classify training examples. Assume the $i$th neuron in our single-layer neural network has output $y_i$, target (desired output) $t_i$, inputs $(x_0, x_1, \ldots, x_N)$, weights $(w_{i0}, w_{i1}, \ldots, w_{iN})$. We want to know how to change a given weight $w_{ij}$, given the error $E_i = (t_i-y_i)$,

\mynote{Derivation of delta rule here}
\begin{equation}
\begin{aligned}
    \frac{\partial E_i}{\partial w_{ij}} &= \frac{\partial E_i}{\partial y_i}\frac{\partial y_i}{\partial w_{ij}} & \textrm{(chain rule)}
\end{aligned}
\end{equation}

\subsection{Backpropogation}
This issue was solved with the discovery of \emph{backpropogation} (also known as the \emph{generalized delta rule}), first demonstrated experimentally by \citet{rumelhartbackprop}, and allowing learning in multi-layer neural networks.

\mynote{Derivation of backpropogation here}
