...
\begin{equation}
\begin{aligned}
    \frac{\partial E^n}{\partial w_{ji}} &= \frac{\partial E^n}{\partial e^n_j}\frac{\partial e^n_j}{\partial y^n_j} \frac{\partial y^n_j}{\partial a^n_j} \frac{\partial a^n_j}{\partial w_{ji}}.& \textrm{(chain rule)}
%    & = -\frac{\partial f\left(w_{ij}x_{j}\right)}{\partial w_{ij}}, & \\
\end{aligned}
\end{equation}
...
The delta rule uses the sensitivity to correct $w_{ji}$ proportionally, 
\begin{equation}
\begin{aligned}
...
where $\gamma$ is a constant called the \emph{learning rate}.

\subsection{Backpropagation}
\begin{figure}[tbp]
\centering
\includegraphics[width=0.8\textwidth]{neurontwolayer}
\caption[An illustration of a neural network with a single hidden layer]{An illustration of a neural network with a single hidden layer. The input layer consists of a set of inputs, $\{x_{i0}, \ldots, x_{iN}\}$. The hidden layer has weights $\{w_{i0}, \ldots, w_{iN}\}$, bias $b_i$, net neuron activity $a_j = \sum_i w_{ji}$ and activation function $f$. The output layer with output $y_k$, has weights $\{w_{k0}, \ldots, w_{jN}\}$ and bias $b_j$.}
...
The following derivation is based on the excellent references of \citet{haykin1994neural,Bishop1995}, although with different notation.
