...
\begin{figure}[tbp]
	\includegraphics[width=0.3\textwidth, page=1, viewport = 0 0 440 600, clip=true]{layer}\hfill
	\includegraphics[width=0.3\textwidth, page=1, viewport = 0 0 440 600, clip=true]{layer}$\cdots$\hfill
	\includegraphics[width=0.3\textwidth, page=1, viewport = 0 0 400 600, clip=true]{layer}
...
unit gaussian activations across all features of the layer
...
\matrix[column sep=0em]{
	\node (1a) {
		\raisebox{-0.5\height}{\includegraphics[height=0.08\textwidth, page=15]{groupfig}}
	};\&
	\node (1b) {
		\raisebox{-0.5\height}{\includegraphics[height=0.11\textwidth, page=21]{groupfig}}
	};\&
	\node (1c) {
		\raisebox{-0.5\height}{\includegraphics[height=0.11\textwidth, page=22]{groupfig}}
	};\\
	...
	\node (2a) {
		\raisebox{-0.5\height}{\includegraphics[height=0.09\textwidth, page=16]{groupfig}}
	};\&
	\node (2b) {
		\raisebox{-0.5\height}{\includegraphics[height=0.11\textwidth, page=8]{groupfig}}
	};\&
	\node (2c) {
		\raisebox{-0.5\height}{\includegraphics[height=0.11\textwidth, page=22]{groupfig}}
	};\\
    ...
    \&\node (3c) {
		\raisebox{-0.5\height}{\includegraphics[height=0.09\textwidth, page=20]{groupfig}}
	};\\
...
\item learning stacked small filters, \ie three 3$\times$3 convolutional layers is more computationally efficient than learning a single convolutional layer of 7x7 filters, and also improves generalization.
\end{enumerate*}.
...
\label{fig:resnet}
\end{figure}
\citet{He2015} introduced residual networks, with the important insight that adding layers to a deep network architecture could in fact \emph{increase} training error. 

\mynote{add more explanation here}

Yet it is not clear why this should be so --- a trivial set of parameters could be learned by the optimization to at least maintain the accuracy of a shallow network, namely the identity. In order to aid optimization, a residual layer is instead trained. Assuming our desired, but difficult to optimize, mapping from one layer to the next is $H(x)$, the residual function learned is simply:
\begin{equation}
	H(x) = F(x) + x.
\end{equation}
