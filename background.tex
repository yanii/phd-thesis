...
Each weight change $\Delta w^n_{ij}$ is a step towards a local minima, backpropagation (like the delta rule) is a method of steepest descent. However $\Delta w^n_{ij}$ is only an approximation to the true gradient we want -- it is only from one sample, $x^n$, of the training dataset $X$. Even if we use the gradient from the entire dataset $X$ (see \cref{batchlearning}), the dataset is only a sampling of the true population we want our network to generalize to. Each gradient will likely move us towards a solution is biased towards the particular subset of samples we calculated it based on.

The learning rate parameter scales the 
\subsection{Momentum}
