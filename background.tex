...
The delta rule for single-layered neural networks is gradient descent, using the derivative of the network's weights with respect to the output error to adjust the weights to better classify training examples. Assume a neuron has output $y$, target (desired output) $t$, inputs $(x_0, x_1, \ldots, x_N)$, weights $(w_0, w_1, \ldots, w_N)$. We want to know how to change a given weight $w_i$, given the error $E = (t-y)$,

\mynote{Derivation of delta rule here}
\begin{equation}
\begin{aligned}
    \frac{\partial E}{\partial w} &= \frac{\partial E}{\partial y}\frac{\partial y}{\partial w} & \textrm{(chain rule)}
\end{aligned}
\end{equation}
