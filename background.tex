...
%
\begin{equation}
	y = \gls{f}\left(\sum_{i}^{N} \gls{w}_i \gls{x}_i + \gls{b}\right),
\end{equation}
%
...
%
\begin{equation}
    y = f\left(\gls{vectorw}^T\gls{vectorx} + b \right).
\end{equation}
%
...
Training is performed on a training dataset $X$, where each training sample $\gls{vectorx}^n\in X$ is a vector $\gls{vectorx}^n = (x^n_0, \ldots, x^n_i, \ldots, x^n_N)$. Assume that for a given training sample $\gls{vectorx}^n$, the $i$th neuron\index{neuron} in our single-layer neural network\index{neural network} has output $y^n_j$, target (desired) output $t^n_j$, weights $\gls{vectorw}=(w_{j0}, w_{j1}, \ldots, w_{jM})$, as shown in \cref{fig:singlelayer}. As shown in the figure, we can consider the bias to be an extra weight with a unit input, and thus we can omit the explicit bias from the derivation. 

We want to know how to change a given weight $w_{ji}$ given the output of node $j$ for a given input data sample $\gls{vectorx}^n$, 
\begin{equation}
...
\Delta\gls{vectorw} &= \gls{vectorw}_t + \mathbf{v}_{t+1},
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
    \Delta w_{ij} &= -\lambda \frac{1}{|X_{\textrm{mb}}|} \sum_{\{n|\,\gls{vectorx}^n \in X_{\textrm{mb}}\}} \frac{\partial E^n}{\partial w_{ji}},
     \label{eqn:sgdrule}
\end{aligned}
\end{equation}
...
Yet from an optimization standpoint it is not clear why this should be so. We can observe that there is a trivial set of parameters defining a transformation that will maintain the training loss of the shallower network, \ie $\mathcal{L}_1 = \mathcal{L}_1$ --- that is the identity transformation $H(\gls{vectorx})=x$. 
