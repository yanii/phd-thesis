...
If all the activation functions $f$ in a multi-layer neural networks were defined as a linear function, a neural network of arbitrary complexity would still have the representational ability of a linear classifier. Activation functions are generally chosen to be a non-linearity. Classically chosen non-linearities were sigmoidal functions, \ie a function mapping negative inputs to negative outputs and positive inputs to positive outputs with a smooth transition around $a = 0$, examples of sigmoid functions commonly used include the logistic function: 
