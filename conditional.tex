...
\begin{document}
	...
	\section{Generalizing \Glsfmttext{dnn} and Decision Trees}
	...
	\begin{figure}[tbp]
		...
		\end{subfigure}
		\caption[Efficiency of conditional networks on \Glsfmttext{ilsvrc}]{\textbf{Efficiency of conditional networks on \glsfmttext{ilsvrc} relative to state-of-the-art.} Our VGG11-GMP (GMP) reduces model size significantly, and is the baseline network. VGG-11 conditional networks (Cond.\ Network)yield points closer to the origin for networks with the same \texttt{conv1} stride (\texttt{conv1}/stride).}\label{fig:Imagenet_results}
	\end{figure}
