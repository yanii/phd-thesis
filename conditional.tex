...
\begin{document}
	...
	\section{Generalizing \Glsfmtshort{dnn} and Decision Trees}
	...
	\begin{figure}[tbp]
		...
		\end{subfigure}
		\caption[Efficiency of conditional networks on \Glsfmtshort{ilsvrc}]{\textbf{Efficiency of conditional networks on \glsfmtshort{ilsvrc} relative to state-of-the-art.} Our VGG11-GMP (GMP) reduces model size significantly, and is the baseline network. VGG-11 conditional networks (Cond.\ Network)yield points closer to the origin for networks with the same \texttt{conv1} stride (\texttt{conv1}/stride).}\label{fig:Imagenet_results}
	\end{figure}
	...
	\begin{figure}[tbp]
		...
		\begin{subfigure}[b]{0.98\linewidth}
			...
			\end{axis}
			\end{tikzpicture}
			\caption{\# \Glsfmtshort{flops} for each layer} 
		\end{subfigure}
		\vfill
		\begin{subfigure}[b]{0.98\linewidth}
			\pgfplotstableread[col sep=comma]{condnetdata/msrc-vgg-11-latelr-layerma.csv}\datatablea
	...
	\subsection{\Glsfmtshort{cifar10}}
	...
	\begin{figure}[tbp]
		...
		\caption[Automatically-learned conditional architecture for \Glsfmtshort{cifar10}]{\textbf{Automatically learned conditional architecture for image classification in \Glsfmtshort{cifar10}.} Both structure and parameters of this conditional network have been learned automatically via Bayesian optimization. \copyright Antonio Criminisi, used with permission.}
	...
	\begin{figure}[tbp] 
		...
		\end{subfigure}
		\caption[Comparing network architectures on \Glsfmtshort{cifar10}]{\textbf{Comparing network architectures on \glsfmtshort{cifar10}.} Conditional networks (denoted with circles) yield the points closest to the origin, corresponding to the best accuracy-efficiency trade-off. Our best conditional network is denoted with a green circle. See text.}\label{fig:Cifar_results}
	\end{figure}
	%
