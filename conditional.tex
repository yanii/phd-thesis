...
\begin{document}
	...
	\begin{figure}[tbp]
		...
		\begin{subfigure}[b]{0.98\linewidth}
			...
			\pgfplotstableread[col sep=comma]{condnetdata/msrc-vgg-11-ns-latelr2-layerma.csv}\datatabled
			\begin{tikzpicture}
			\begin{axis}[		
				axis x line=bottom,
				axis y line=left,
			...
			\addplot table [x expr=\coordindex,y=MA]{\datatablec};
			\legend{VGG-11, msrc-vgg-11-ns conv5 512, Conditional Network,VGG11-GMP};
			\end{axis}
			\end{tikzpicture}
		\end{subfigure}
		~
		\begin{subfigure}[b]{0.98\linewidth}
			...
			\addplot table [x expr=\coordindex,y=param]{\datatabled};
			\end{axis}
			\end{tikzpicture}
		\end{subfigure}
		% \caption[Conditional networks layer-wise FLOPS/parameters]{\textbf{Conditional Network Layer-wise \gls{flops}/Parameters.} 
		% }
		\caption[Computational cost of VGG-11 based networks per layer]{\textbf{Computational cost of VGG-11 based networks per layer.} Number of parameters and number of multiply-accumulate operations for all (convolutional and fully-connected) layers of VGG11, VGG11-GMP, and our conditional network. Our two networks reduce both the memory use and the computational cost.}\label{fig:VggPerLayerCost}
	\end{figure}
