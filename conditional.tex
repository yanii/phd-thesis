...
\begin{document}
	...
	are the initial learning rate, learning rate at iteration $t$, and weight decay\index{weight decay} respectively~\citep{Bottou2012sgdtricks}. 
	...
	\begin{figure}[tbp]
		...
		\end{subfigure}
		\caption[Efficiency of conditional networks on \gls{ilsvrc}]{\textbf{Efficiency of conditional networks on \gls{ilsvrc} relative to state-of-the-art.} Our VGG11-GMP (GMP) reduces model size significantly, and is the baseline network. VGG-11 conditional networks (Cond.\ Network)yield points closer to the origin for networks with the same \texttt{conv1} stride (\texttt{conv1}/stride).}\label{fig:Imagenet_results}
	\end{figure}
	...
	\begin{figure}[tbp]
		...
		\begin{subfigure}[b]{0.98\linewidth}
			...
			\end{axis}
			\end{tikzpicture}
			\caption{\# \Gls{flops} for each layer} 
		\end{subfigure}
		\vfill
		\begin{subfigure}[b]{0.98\linewidth}
			\pgfplotstableread[col sep=comma]{condnetdata/msrc-vgg-11-latelr-layerma.csv}\datatablea
