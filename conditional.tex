...
\begin{document}
	...
	\begin{figure}[tbp]
		...
		\begin{subfigure}[b]{0.98\linewidth}
			...
			\addplot[mark=*,mark options={fill=green},
			nodes near coords,
			only marks,
			...
			\addplot[mark=diamond*,mark options={fill=green},
			nodes near coords,
			only marks,
			point meta=explicit symbolic,
			every node near coord/.append style={xshift=0.01em, anchor=west, font=\scriptsize},
		...
		\end{subfigure}
		\caption[Efficiency of conditional networks on \gls{ilsvrc} relative to state-of-the-art]{\textbf{Efficiency of conditional networks on \gls{ilsvrc} relative to state-of-the-art.} Our VGG11-GMP (GMP) reduces model size significantly, and is the baseline network. VGG-11 conditional networks (Cond.\ Network)yield points closer to the origin for networks with the same \texttt{conv1} stride (\texttt{conv1}/stride), corresponding to the best accuracy-efficiency trade-off.}\label{fig:Imagenet_results}
	\end{figure}
