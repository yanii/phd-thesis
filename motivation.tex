...
\begin{equation}
	p(\mathbf{\theta}|\mathcal{M}_i,D) = \frac{p(D|\mathbf{\theta},\mathcal{M}_i) p(\mathbf{\theta}|\mathcal{M}_i)}{p(D|\mathcal{M})},
\end{equation}
...
The \emph{prior}\index{prior}, $p(\mathbf{\theta}|\mathcal{M}_i)$, are the probabilities on what values we expect the weights of the model to be. The normalization term, $p(D|\mathcal{M}_i)$, is the probability of the data given the model --- what \citeauthor{mackay1992practical} calls the `evidence' for the model $\mathcal{M}_i$.
...
In fact \citeauthor{mackay1992practical} points out that the Bayesian approach encodes the \gls{occam} approach.
