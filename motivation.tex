...
\begin{document}
    ...
    In the case of neural networks, early work showed that the capacity of neural networks to be quite large~\citep{hornik89a,baum1989size}. \citet{baum1989size} looked at feed-forward networks of \emph{threshold units}, \ie perceptrons\todo{is this what they are?}, showing the VC dimension of a single-layer network with M units and W weights (including biases) to be bounded by,
    \begin{equation}
        d_{\textrm{VC}} \leq 2 W \log_2(eM),
    \end{equation}
    
    where $e$ is the mathematical constant, the base of natural logarithms. For 2 layers, they found a lower bound,
    \begin{equation}
        d_{\textrm{VC}} \geq 2 \lfloor M/2 \rfloor d,
    \end{equation}
    where $\lfloor \ \ \rfloor$ is the floor operation, \ie largest integer less than or equal to the operand, and $d$ is the number of inputs. At the time, the largest number of weights in a network were typically on the first layer, and the authors make the assumption that $Md\approx W$. With this assumption, they who
    
    \todo{expand here! put some math}
