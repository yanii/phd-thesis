...
\section{Growing Neural Network Architectures}
    ...
    This approximation is used directly as the saliency measure for each network weight $w_i$, and after having trained the network, the weights are sorted by saliency, and the lowest saliency parameters are deleted.
    ...
    In order to use deep neural networks on embedded devices, and in many applications, first a compact representation is required. There have been many papers on compressing deep networks, notably the work of ~\citep{han2016dsd} looked at combining a simple weight magnitude-based pruning method with both quantization and Huffman coding, in all the method can take a trained network
