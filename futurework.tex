...
In my PhD, I have focused on experiments which I believed would shed light on the representations being learned in \glspl{dnn}\index{DNN}. Although the overt motivation of much of the work in its publication has been efficiency, my personal motivation has always been to show that state-of-the-art \glspl{dnn}\index{DNN} for image classification are over-parameterized and suffer, not only in efficiency, but also in \emph{effectiveness} due to this. Structural priors, such as those demonstrated in this dissertation do not only improve the effectiveness of a deep network, but are \emph{required} for good generalization.
\end{document}
