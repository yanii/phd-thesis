...
\begin{document}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            &= f^\textrm{weights}_{il} \sum_{l} \gls{fmY}^\textrm{basis}_l \quad\textrm{(since $\gls{fmK}^\textrm{weights}_{il}$ is a scalar)}\\
            &= f^\textrm{weights}_{il} \left( \sum_{l=0}^{m/2} \gls{fmX}_l \gls{convolution} \gls{fmK}^\textrm{h}_{il} + \sum_{l=m/2}^m \gls{fmX}_l \gls{convolution} \gls{fmK}^\textrm{v}_{il}\right),
        \end{aligned}
    \end{equation}
