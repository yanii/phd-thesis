...
\begin{document}
    ...
    \end{equation}
    Here $\mathbf{y}_l$ is a $d$$\times$$1$ vector representing a pixel in the output \gls{featuremap}\index{feature map} and $\bf x_l$ is a $ w h c \times 1$ vector that represents a $w$$\times$$h$ sub-region of the $c$-channel input \gls{featuremap}\index{feature map}. $\mathbf{W}_l$ is the $d$$\times$$n$ weight matrix, where $d$ is the number of filters and $n$ is the size of a filter, \ie $n = w h c$ for a filter with spatial dimensions $w$$\times$$h$ operating on an input \gls{featuremap}\index{feature map} of $c$ channels, and $\mathbf{b}_l$ is the bias. Finally $\mathbf{x}_l = f(\mathbf{y}_{l-1})$ is the output of the previous layer passed through an activation function $f$ (\eg the application of a \gls{relu} to each element of $\mathbf{y}_{l-1}$).
    ...
    \end{equation}
    where now $\Delta \mathbf{y}^{[i]}$ represents $w^{[i]} \times h^{[i]}$ pixels in $d^{[i]}$ channels of the output \gls{featuremap}\index{feature map}. Each $\hat{\mathbf{W}}_l^{[i]}$ is a $c_l \times \hat{n}^{[i]}$ matrix of weights arranged appropriately for back propagation. Again, note that each $\hat{\mathbf{W}}_l^{[i]}$ can be simply reshaped from $\mathbf{W}_l^{[i]}$.
