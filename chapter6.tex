...
\end{axis}
\end{tikzpicture}
\caption{Change in top-5 training acc.~for filter pairs}
...
\end{axis}
\end{tikzpicture}
\caption{Change in top-5 validation acc.~for filter pairs}
...
\caption{Histogram of change in top-5 validation acc.}
\label{fig:resnet50ablation_conv1_top5_test}
\end{subfigure}

%\input{resnet50fig.tex}
\caption{Histogram of the change in top-5 accuracy for all pairwise filter ablations of a 2500 randomly sampled images from the ILSVRC training/validation set in \texttt{conv1} of ResNet-50.}
...
\label{fig:mnist_ablation_stats_train_loss}
\end{subfigure}
